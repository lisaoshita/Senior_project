...
\section{Exploratory Analysis and Feature Engineering}
...
Table \ref{table:countries} highlights how imbalanced the target destinations are. NDF and US
account for almost 90\% of the entire data, indicating that a majority of users either 
have yet to make a booking, or book locations in the US. The remaining nine, under-represented 
