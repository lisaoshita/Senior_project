...
# mean close to 0
sapply(train_users[, 43:ncol(train_users)], mean)
# median = 0
sapply(train_users[, 43:ncol(train_users)], median)
# sd close to 0
sapply(train_users[, 43:ncol(train_users)], sd)

# set ranges: if num_action is greater than or less than 1
action_bucket$num_index_bucket <- cut(train_users$num_index, 
                                      breaks = c(min(train_users$num_index),
                                                 1, max(train_users$num_index)),
                                      right = FALSE)
...
# cols: 18-24, 26, 28-32, 34-41
to_encode <- train_users[, c(18:24, 26, 28:32, 34:41)] 
to_encode[is.na(to_encode)] <- -1 # converting all NAs to -1 (gbm doesn't work with NAs)
train <- cbind(train_users$country_destination, 
               data.frame(predict(dummyVars("~.", data = to_encode),
                                  newdata = to_encode)))
train <- train[-213452, ] # removing row with NA for country_destination
library(gbm)
boostAirbnb <- gbm(train$`train_users$country_destination` ~ ., data = train, 
                   distribution = "multinomial", n.trees = 5, interaction.depth = 4)
boostAirbnb
predictions <- predict(boostAirbnb, newdata = train, n.trees = 5, type = "response")

train$predictions <- apply(predictions, 1, which.max)
levels <- data.frame(countries = levels(train$`train_users$country_destination`), num = 1:12)

train$predictions[train$predictions == 1] <- "AU"
train$predictions[train$predictions == 2] <- "CA"
train$predictions[train$predictions == 3] <- "DE"
train$predictions[train$predictions == 4] <- "ES"
train$predictions[train$predictions == 5] <- "FR"
train$predictions[train$predictions == 6] <- "GB"
train$predictions[train$predictions == 7] <- "IT"
train$predictions[train$predictions == 8] <- "NDF"
train$predictions[train$predictions == 9] <- "NL"
train$predictions[train$predictions == 10] <- "other"
train$predictions[train$predictions == 11] <- "PT"
train$predictions[train$predictions == 12] <- "US"

# 100% accuracy?? 
sum(train$predictions == train$`train_users$country_destination`) / nrow(train)

pred <- data.frame(country = train$`train_users$country_destination`, predictions = train$predictions, 
                   correct = train$`train_users$country_destination` == train$predictions)
```
