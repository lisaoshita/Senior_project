...
to_encode <- train_users %>% select(-c(starts_with("num_"), starts_with("d_"), starts_with("ad_"),
                                       ...
                                       firstactive_date, firstbook_date, book, lat_destination, lng_destination))
...
# ===========================================================================
# gradient boosted model with basic vars (variables given in data set)
pract <- train_users[, 7:14]
pract_train <- cbind(train_users$country_destination,
                     data.frame(predict(dummyVars("~.", data = to_encode),
                                  newdata = to_encode)))
colnames(pract_train)[1] <- "country_destination"

boostpract <- gbm(country_destination ~., data = pract_train, distribution = "multinomial",
                  n.trees = 5)
predictions <- predict(boostpract, newdata = pract_train, n.trees = 5, type = "response")
predict <- apply(predictions, 1, which.max) 
