...
df_t <- t(df)
colnames(df_t) <- df$action
df_t <- df_t[-1, ]
names(df_t)
unname(df_t)


unique_action <- unique(sessions$action)
for (act in unique_action) {
  user_sub <- sessions %>% filter(user_id == "d1mm9tcy42")
  count_act <- user_sub %>% count(action)
}
