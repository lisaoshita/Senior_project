...
# creating ranges 
# ===========================================================================
# mean, median, sd for each count close to 0
...
get_counts <- function(user, var) {
  df <- sessions %>% filter_(as.name(user_id) == user) %>% count_(as.name(var)) # subsetting rows 
  # if (nrow(df) <= 1) {
  #   df_t <- as.data.frame(data.table::transpose(df))
  #   df_t$REMOVE_LATER <- NA # adding extra column (to retain df class)
...
sessions %>% summarise_(n_uniq = interp(~n_distinct(v), v = as.name(uniq.var)))

get_counts(user = "d1mm9tcy42", var = "action_type")
get_type_counts("zzvatt4dio", var = "action_type") # not working
sessions %>% filter(user_id == "d1mm9tcy42") %>% count(action_type) # subsetting rows

library(lazyeval)

df <- sessions %>% filter(user_id == "yo8nz8bqcq") %>% count(action_type)
df <- sessions %>% filter(user_id == "d1mm9tcy42") %>% count(action)
df_t <- as.data.frame(data.table::transpose(df))
df_t$REMOVE_LATER <- NA
df_t
colnames(df_t) <- c(paste("num", df$action_type, sep = "_"), "REMOVE_LATER")
df_t <- df_t[-1, ]
ncol(df)
nrow(df)
...
```{r}
d <- data.frame(unknwon = 1)
data.table::transpose(d)
```
